...
'/Users/aryandeore/Downloads/WalmartSalesData.csv'
INTO TABLE sales
...
SET product_category = 
  CASE 
    WHEN total >= (SELECT AVG(total) FROM (SELECT total FROM sales) AS t) THEN 'Good'
    ELSE 'Bad'
  END;
...
SELECT branch,
       day_name,
       AVG(rating) OVER(PARTITION BY branch) AS avg_rating
FROM sales
